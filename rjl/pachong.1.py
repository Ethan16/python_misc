...
#     '''
#     print u'该站使用的协议是：' + re.findall(r'.*(?=://)',url)[0]
...
def processUrl(self,url):
    '''
    判断正确的链接及处理相对路径为正确的完整url  加上http://域名
    :return:
    '''
    true_url = []
...
def getinfo(webaddress):  
    ...
    # -r递归下载
    command = 'wget -r -m -nv '+' -o '+outputfilepath+' '+url#利用wget命令爬取网站  
    print command
    ...
    target_url = re.findall(r'(?<=href=\").*?(?=\")|(?<=href=\').*?(?=\')',allinfo)
    target_url2=[]
    #过滤列表中所有的css链接,空项
    for t in target_url:
        if re.match(r'.*?(?=.css)',t) or t=='':
            continue
        target_url2.append(t)
    ...
    for i in range(target_num):  
        print i 
        print >> fobj1,target_url[i][0:]  #将结果写入文件
    fobj.close()  
    fobj1.close()  
